...
class Money {
  ...
  multiply = (value: number | string, multiple?: number, decimal?: number): number => {
    ...
    if (value !== '' && value !== null && value !== undefined) money = parseFloat((+value * meneyMulty).toFixed(moneyDecimal))
    if (!isNaN(money)) return money
    else return 0
  }

  divide = (value: number | string, multiple?: number, decimal?: number): { value: string; real: number; origin: number } => {
    const moneyDecimal: number = decimal ?? this.dec
    const meneyMulty: number = multiple ?? this.multy
    ...
    if (value !== '' && value !== null && value !== undefined) {
      plain = parseFloat((+value / meneyMulty).toFixed(moneyDecimal))
